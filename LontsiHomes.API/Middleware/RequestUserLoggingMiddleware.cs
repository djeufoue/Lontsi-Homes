...
namespace LontsiHomes.API.Middleware
{
    public class RequestUserLoggingMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            ...
            _logger.LogInformation(
                "API request started {Method} {Path} by {UserEmail} ({UserId})",
            ...
            finally
            {
                stopwatch.Stop();
                _logger.LogInformation(
                    "API request completed {Method} {Path} with {StatusCode} in {ElapsedMilliseconds}ms by {UserEmail} ({UserId})",
                    context.Request.Method,
                    context.Request.Path,
